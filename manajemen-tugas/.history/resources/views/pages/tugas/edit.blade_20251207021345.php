            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="Proses" {{ old('status', $tugas->status) == 'Proses' ? 'selected' : '' }}>Proses</option>
                        <option value="Sedang" {{ old('status', $tugas->status) == 'Sedang' ? 'selected' : '' }}>Sedang</option>
                    ...
                    <select name="kategori"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ old('kategori', $tugas->category_id) == $category->id ? 'selected' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (kategoriSelect.value === 'buat_baru' || '{{ old("kategori_baru") }}') {
            wrapper.style.display = 'block';
            input.required = true;
            kategoriSelect.value = 'buat_baru';
        }
        ...
        kategoriSelect.addEventListener('change', function() {
            if (this.value === 'buat_baru') {
                wrapper.style.display = 'block';
                input.required = true;
                // Kosongkan value select agar tidak terkirim
                this.value = '';
            } else {
        ...
        // Prevent "buat_baru" from being submitted
        document.querySelector('form').addEventListener('submit', function(e) {
            if (kategoriSelect.value === 'buat_baru') {
